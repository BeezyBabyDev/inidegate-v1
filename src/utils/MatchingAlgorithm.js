...
export class MatchingAlgorithm {
  ...
  calculateTalentProjectMatch(talentProfile, projectOpportunity) {
    ...
    const skillsScore = this.matchSkills(talentProfile.skills, projectOpportunity.requirements?.skills || [])
    score += skillsScore * this.weights.skills
    if (skillsScore > 0.8) reasons.push(`Perfect skill match: ${this.getTopSkillMatch(talentProfile.skills, projectOpportunity.requirements?.skills)}`)
    else if (skillsScore > 0.6) reasons.push(`Strong skill alignment`)

    // 2. Location Matching (15% weight)
    const locationScore = this.matchLocation(talentProfile.location, projectOpportunity.location, talentProfile.remoteWork)
    ...
    const experienceScore = this.matchExperience(talentProfile.experience, projectOpportunity.requirements?.experience)
    ...
    const availabilityScore = this.matchAvailability(talentProfile.availability, projectOpportunity.startDate)
  ...
  calculateInvestorTalentMatch(investorProfile, talentProfile) {
    ...
    const budgetScore = this.matchInvestmentRange(investorProfile.investmentRange, talentProfile.seekingFunding)
    ...
    score += genreScore * 0.20
    if (genreScore > 0.8) reasons.push(`Genre preference match: ${this.getCommonGenres(investorProfile.preferredGenres, talentProfile.genre).join(', ')}`)

    // 3. Track Record & ROI Potential (25% weight)
    const trackRecordScore = this.evaluateTrackRecord(talentProfile.achievements, talentProfile.credits)
    ...
    const stageScore = this.matchInvestmentStage(investorProfile.investmentStage, talentProfile.projectStage)
  ...
  matchInvestmentRange(investorRange, seekingAmount) {
    ...
    const investorMax = parseInt(investorRange.split('-')[1]?.replace(/[^0-9]/g, '') || investorMin * 10)
  ...
  matchInvestmentStage(investorStages = [], projectStage) {
    ...
    return investorStages.some(stage => 
  ...
  estimateROIPotential(talentProfile) {
    ...
    const hasAwards = achievements.some(a => a.toLowerCase().includes('winner') || a.toLowerCase().includes('award'))
    const hasFestival = achievements.some(a => a.toLowerCase().includes('sundance') || a.toLowerCase().includes('cannes'))
  ...
  filterMatches(matches, filters) {
    return matches.filter(match => {
      if (filters.location !== 'all' && !match.location.toLowerCase().includes(filters.location)) return false
      if (filters.genre !== 'all' && !match.genre?.some(g => g.toLowerCase().includes(filters.genre))) return false
